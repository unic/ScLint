...
| 12. | ScLint12 | Use <em>IsNullOrEmpty()</em>, <em>IsNullOrWhiteSpace()</em> methods and <em>is</em> operator instead of <em>Equals Expression (==)</em> when checking whether object is null | All <em>Equals Expressions</em> and <em>Not Equals Expressions</em> are examined in terms of checking whether an object is null. When such comparison is made with <em>equal</em> or <em>not equal</em> operators analyzer suggests to change it to <em>IsNullOrEmpty()</em> or <em>IsNullOrWhiteSpace</em> method in case of strings or to <em>is</em> operator in case of comparing other objects. The rule recognizes when there are negations and modifies the code part to match preceding expression.<br>Type: warning  | NullStrings |
